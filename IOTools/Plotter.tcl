...
#        drawGraphpad $w
	...
	if {$plot($winId,timeFmt) ne "General"} {
	    ...
	    foreach bound {Xmin_data Xmax_data} {
		set plot($winId,$bound) [expr {$plot($winId,$bound)*$unitInDays}]
	    }
	}
...
################################################################################
        ...
        pack [set xaxix [ttk::labelframe $chkF.xaxis -text "X-axis scale"]] \
	    -fill x
        pack [ttk::checkbutton $xaxix.cbutton -text [tr. "Fewer ticks"] -variable [namespace current]::FewXAxisTicks]
	pack [ttk::frame $xaxix.timeFmts]
	foreach {label effect} {number General date YYYYMMDD time HHMMSS} {
	    pack [ttk::radiobutton $xaxix.timeFmts.$label -text $label \
...
################################################################################
	...
	set sCol $::looks(outlineColor)
        ...
        $w.canvas create text [expr $x0+$plot($w,xlength)/2.0] $y0 \
                -text "\nTime" -fill $sCol -anchor n \
                -tags {movable scalable xaxis_label markable toplevel}
        $w.canvas create text $x0 [expr $y0-$plot($w,ylength)/2.0] -fill $sCol \
...
#         update
	...
	set sCol $::looks(outlineColor)
		...
		-text [string map {\n { }} [lindex $plot($w,Ylabels) $i]] \
                    -fill $sCol -anchor nw -font $plot($w,fontLabels)\
                    -tags [list $vartag legend markable toplevel]
            incr j
            if {$j==2} {
                incr k
    ...
    proc resize {w win x y width height} {
        ...
        if {[string match Canvas [winfo class $win]]} {
            ...
            set new_height [expr {max(2*round(($height-$plot($w,yborder_top)- \
					      $plot($w,yborder_bottom))/2),2)}]
