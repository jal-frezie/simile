...
defines_membership(SmByRec, Fp) :-
	...
	(is_parameter(Comp, 2), \+ Comp is_of_sort discrete,
	 \+ (Comp has_class_refinement value of Val, \+ Val == ''),
	 Fp = Comp;
...
%	V is SimV + 4,
	...
	sicstus_format_to_chars("\"program='Simile',version=~f,edition=~a,date=~s,size=~d,\"", [SimV, Edition, DateStr, FnCount], IdentStr),
...
%	    append(ReproConds, ImmigConds, NewMemConds),
	    ...
	    append([[make(can_enter(Name),
		    ...
		    make(enumerate(Name), [/* evt_culled(Name), */ can_enter(Name), bred(Name) | Losses
					   ], LocalPath, Step, []),
