...
do_assignment(L, [define_proc_for(Sm, Path), open_index(MSt, _B) | Clauses],
	...
	[unify(L), unify(mThd), build([tid, phase, context]),
	 build(_Nms), build([TidMem, PhaseMem, ContextMem])]),
    ...
    SetupRandCall =.. [setup_thread, TidMem],
    ...
    make_procedure_call_chars(L, [pipeRead, '(char*)snf',
    ...
    excrete(L, procedure_call, pipeWrite('(char*)snf',
...
do_assignment(L, [call_proc_for(Sm, Path, Loop, [Td, Tk, Tr]) | Clauses],
    ...
    append_atoms('struct ', ContextName, StructType),
...
/*	excrete(L, variable_declaration, [Type, Pointer, []], Indent, Stream),
	...
	(setof(LossVal, get_term_refs(L, Pointer, LossNodes, LossVal),
	    ...
	    refer_value(L, phase, Phase),
	    make_evaluation_routine(L, dt(Phase), Used, CanDie),
