...
update_equation(Function, InterInputs, [Eqn_st, Unit_pb, Is_P_st, Desc_st,
					Cmt_st, Min_st, Max_st],
		Effect) :-
...
test_eqn(Equation, Fn, IndxCount, InterInputs, Type, Dims,
	...
	(nonvar(ParseError), !;
	 ...
	 purge(UsedList, ['/tm/','/dest/'], ParamList),
	    ...
	    get_actual_sizes(Fn, NearlyDimDG, bare, DimDG, _V, _U),
	    append(DimDG, _, DimL),
	    length(DimDG, LenD),
	    length(DimDV, LenD),
