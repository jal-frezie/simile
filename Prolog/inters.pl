...
/*
	    ...
	    (Source = sofar(_), !,
	     ...
	     % size -- a need that no longer exists.
	        Wrapper = later,
...
%	  InterPath = OrigPath),
	...
	using made_at(...) should prevent it being removed as an idler */
	 member(Source-Delay-Xfer, [in_preceding(_)-this_loop-[],
	 % preceding value must be created in same loop in which it is used
			       in_progenitor(_)-this_loop-[_]]), !,
	 ...
	 % offspring in same step as parent -- access before setting
	    ...
	    append(Xfer, BasePath, DestPath),
	    % lower made_at() a level to signify that because progenitors are
	    % always made first we might as well be looking in the parent
	    Args = [made_at(cleared(Name), SourceContext),
 				% array must be cleared before first use
		    made_at(DelayCond, BasePath)];
