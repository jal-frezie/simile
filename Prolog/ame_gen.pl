...
make_legible_for_prolog(String, NewString, CommentsAllowed) :-
	...
	starter_only(StartsVar, prolog, StartsVar),
	    ...
	    % lower([StartsVar | MoreVar], LowerVar), converts all funx to lower
	    % including macro defns which then don't match old specs :-(
	    LowerVar = [StartsVar | MoreVar], 
...
find_ghosts(Base, Ghost) :-
	...
	sequence(Base, Ghost));
	ghost_link(Link, Base, Ghost),
	Link is_connector from _ to Ghost.
				% make sure only one solution per B/G pair
...
Obj is_class_of_sort Class :-
	member(Obj-SortList,
		...
		state-[rectangle, tall_box, box, has_function,
		       init_eval, continuous, can_be_ghost],
		submodel-[rounded_rect, elongated_box, box],
		flow-[line, has_function, has_bowtie, rate, transfer],
		squirt-[line, has_function, has_bowtie, discrete, transfer],
		...
		creation-[regular_box, box, rectangle, channel, has_function,
			  init_eval, level, value_outside, pop_only, generator],
