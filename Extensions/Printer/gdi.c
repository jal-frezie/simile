...
**
** Thanks to Viktor Björk <viktor@electronicfarm.com> and
...
**
** Thanks to Carlos Tasada <carlos.tasada@farmerswife.com> for
** slight modification for math function use in transparencies
**
    ...
    for (k=1; k<argc; k++)
    {
      if ( strcmp(argv[k], "-window") == 0 )
      {
        ...
        else
        {
          ...
          wnd = (HWND)PTR_FROM_STR(argv[++k], &strend, 0);
...
static HDC get_dc(Tcl_Interp *interp, const char *name)
{
  ...
  if ( hdc_loaded == 0 || hdc_valid == 0 || hdc_valid(interp, name, -1) == 0 )
  {
    ...
    tmp = PTR_FROM_STR(name, &strend, 0);
