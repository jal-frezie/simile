...
char* licenseRight (Tcl_Interp *interp) {
  ...
  const char* offered = Tcl_GetVar2(interp, "env", "license_code", 0);
  if (!strlen(offered))
    return "evaluation";
  ...
  for (count=0; count<3; ++count) {
    ...
    Tcl_AppendStringsToObj(dataCombo, "%", editions[count], "^", secret, NULL);
    if (my_hash(interp, dataCombo) == TCL_ERROR) {
      return NULL;
    }

    /* check it matches what we got before */
    if (!strncmp(offered, Tcl_GetStringResult(interp), 10)) {
