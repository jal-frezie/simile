...
FINDABLE EXPORT int do_evalmodel(InstanceOfModel* handle, int phase) {
  ...
  handle->dts[0] = phase; // so external code can access it
  error = setjmp(env);
  if (error) {
    return -error;
  } else {
    try {
      handle->evalmodel(phase);
    }
    catch (int error) {
      return error;
    }
    catch (InternalStop notice) {
      handle->userStop.targetId = notice.lineNo;
      handle->userStop.excpNo = notice.userCode;
    }
    return handle->userStop.excpNo;
  }
}
...
FINDABLE EXPORT int do_setstep(InstanceOfModel* handle, 
			       double time, int phase) {
  if (phase>0) { /* lazy */
    handle->dts[phase] = time;
  } else {
    handle->ts[-phase] = time;
  }
  return(phasecount);
}
