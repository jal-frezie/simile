...
proc FillTable { t pairs } {
	# Initialize variables and display first usable list
	$t.middle.alist delete 0 end
	$t.middle.vlist delete 0 end

	foreach avPair $pairs {
		$t.middle.alist insert end [lindex $avPair 0]
		$t.middle.vlist insert end [lindex $avPair 1]
	}
}

proc attributeBindings { t ea ev lba lbv ok can } {
	# t - toplevel
	# ea - attribute entry box
	# ev - value entry box
	# lba - attribute listbox
	# lbv - value listbox
	# ok - OK button
	# can - Cancel button
	...
	foreach w [list $ea $ev $lba $lbv $ok $can] {
	...
	# bind $t <Control-c> attributeCancel

	# Entry bindings
	bind $ev <Return> attributeOK

	# A single click, or <space>, puts the name in the entry
	bind $lba <space> "attributeTake $%W  attribute(aname); focus $ea"
	bind $lba <Button-1> \
		"attributeClick %W %y  attribute(aname); focus $ea"
	bind $lbv <space> "attributeTake $%W  attribute(vname); focus $ev"
	bind $lbv <Button-1> \
		"attributeClick %W %y attribute(vname); focus $ev"
...
proc attributeOK {} {
	global attribute
	set attribute(done) 1
}

proc attributeCancel {} {
	global attribute
	set attribute(done) 1
	set attribute(aname) {}
	set attribute(vname) {}
