...
set msgs(edited_by) [tr. {Edited by %1$s on %2$s}]
...
set msgs(trigonometry) [tr. {Trigonometrical functions}]
set msgs(typed_submodels) [tr. {Functions used in association with special types of multi-instance submodel, such as those representing hexagonal grids}]
set msgs(fragments) [tr. {Functions defined by model fragments, which are implemented by inserting the fragment into the model when it is built}]
...
set msgs(reverse) [tr. {Reverses the order of elements in an array}]
...
set msgs(sporadic) [tr. {Defines an event that happens at random points in time with the given mean frequency, which can itself vary over time. Only to be used as the whole equation of a derived event with no antecedents.}]
...
set msgs(order) [tr. {Returns an array holding the indices of the argument array in ascending order of the element values}]
...
set msgs(tlPopups) [tr. {Use top-level mini-windows, which can extend outside their host windows, for popups, tooltips and drop-down option lists. This is the default except on the Mac, where it can cause problems on multi-screen displays.}]
...
set msgs(by_shared_sizes) [tr. {Include only values from submodels with matching indices along all shared dimensions}]
...
set msgs(metafile_ref) [tr. {Loaded from file "%1$s" according to reference in %2$s}]
...
set msgs(writing_decls) [tr. {Writing model structure information}]
...
%2$s <HERE> %3$s}]
set msgs(prolog_bug) [tr. {Unexpected Prolog error message: "%s"}]
...
set msgs(wrong_param_type_title) [tr. {Parameter %4$s incompatible}]
set msgs(wrong_param_type_message) [tr. {The definition of explicit intermediate result "%1$s" gives it %4$s %2$s, which are incompatible with the context in which it is used recursively in its own definition, requiring %4$s that can be used as %3$s.}]
...
set msgs(trigger_units_undefined_title) [tr. {Trigger units unavailable}]
set msgs(trigger_units_undefined_message) [tr. {There are no events that trigger the derived event "%1$s" so it is impossible to decide the units or dimensions of the trigger magnitude to use in the equation.}]

set msgs(no_antecedents_for_derived_title) [tr. {Nothing to trigger %1$s}]
set msgs(no_antecedents_for_derived_message) [tr. {The derived event "%1$s" does not have influences from any other events, so it never occurs.}]

set msgs(no_triggering_events_title) [tr. {Nothing to trigger %1$s}]
set msgs(no_triggering_events_message) [tr. {The rule-based state variable "%1$s" does not have influences from any events, so it never changes from its initial value.}]
...
set msgs(absent_enum_type_message) [tr. {Cannot resolve reference to size of %1$s in component %2$s. There is no local enumerated type definition that includes this name.}]
...
set msgs(unknown_unit_detail) [tr. {Note that unit abbreviations and multiplier prefixes are case-sensitive.}]
...
set msgs(needs_loss_parameter_title) [tr. {Argument must be loss channel}]
set msgs(needs_loss_parameter_message) [tr. {The argument of "dies_of" must be a value from a loss channel for the population submodel containing its node. "%1$s" is not this sort of argument.}]
...
set msgs(reserved_et_member_title) [tr. {Reserved %1$s name}]
set msgs(reserved_et_member_message) [tr. {"%2$s" is reserved for the system enumerated type "boolean", which has values of "false" or "true".}]
...
set msgs(moved_component_title) [tr. {Some parameter values unused}]
set msgs(moved_component_message) [tr. {The %1$s contains %2$s for the %3$s "%4$s", which does not exist in the target model "%5$s".}]
set msgs(moved_component_detail) [tr. {Do you want to ignore these values and continue loading the %1$s?}]
set msgs(moved_component_full) [tr. {You may also choose to select another %3$s which will get these %2$s instead of "%4$s", for instance if it has been renamed since saving the parameters.}]
...
set msgs(too_much_data_message) [tr. {You have entered too much data into this dialogue. The practical limit is about 8000 characters.}]
...
set msgs(model_stuck_detail) [tr. {Forcing the model to stop may leave it in an inconsistent state. If you continue to wait, this message will close when the model completes the current time step.}]
...
set msgs(relocate_button) [tr. {Find elsewhere}]
set msgs(redirect_button) [tr. {Use elsewhere}]
