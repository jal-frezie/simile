source(program='Simile v7.0',version= 11.0,edition=enterprise,date='Fri May 12 11:56:25 GMT 2023').

roots([node00001,node00003,node00004,node00014,node00104,node00202,node00203,node00204,node00205,node00206]).
...
node(node00001,variable,[],[complete=true,max_val= 1.0,min_val= 0.0,name=mag,units=1],[caption_offset=[0,0],centre=[180,76]]).
node(node00003,event,[],[complete=true,name=size],[caption_offset=[0,0],centre=[423,64]]).
...
node(node00104,function,[],[complete=true,name=fn102,spec=mag,units=1,value=mag],[]).
...
node(node00013,function,[],[complete=true,name=fn6,spec='max(exprnd(1),1e-9-latency)',units=1/1,value=max(exprnd(1), 1.0e-09-latency)],[along=390]).
node(node00015,function,[],[complete=true,name=fn7,spec=freq,units=1/day,value=freq],[along=550]).

arc(arc00102,node00104,node00003,influence,[attached=[],name=i102],[]).
...
arc(arc00103,node00001,node00104,influence,[attached=[],complete=true,name=i103,role=[use(none,in_hierarchy,mag,1)]],[curve=[-2,-38]]).
arc(arc00104,node00202,node00104,influence,[attached=[],complete=true,name=i104],[curve=[-11,-7]]).
