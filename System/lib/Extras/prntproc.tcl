...
#
# $Log: prntproc.tcl,v $
# Revision 1.1  2005/04/06 09:44:39  jmm
# Moved the left margin -400 units in proc print_page_nextline
#
...
proc page_args { array } {
  ...
  if { ( [ info exist ary(hDC) ] == 0 ) || ($ary(hDC) == 0x0) } {
    ...
    #ShowMessage debug info "ary(tm) ary(bm) ary(lm) ary(rm) : $ary(tm) $ary(bm) $ary(lm) $ary(rm)" ok
...
proc print_page_nextline { string carray parray y font } {
  ...
  set maxwidth [ expr ( ( $printargs(pw) - $printargs(rm) ) / 1000 ) * $printargs(resx) ]
    ...
    set lm [ expr $printargs(lm) * $printargs(resx) / 1000 - 400]; # todo jmm added -400
