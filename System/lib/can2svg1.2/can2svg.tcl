...
#   1.2      various elaborate tweaks specific to needs of Simile (but all good)
#
# ########################### TODO #############################################
# 
...
proc can2svg::MakeArcPath {coo opts} {
    ...
    ## JAT botch to fix near-360-degree arcs so they always appear rather than
    ## getting rounded to full circles, which don't
    if {[format "%.3f %.3f" $x1 $y1] eq [format "%.3f %.3f" $x2 $y2] && \
	    fmod($optA(-extent),360)!=0} {
	set y2 [expr {$y2+0.001}]
    }
	    
    set fa [expr {abs($delta) > $pi} ? 1 : 0]
    set fs [expr {$delta > 0.0} ? 1 : 0]
    
    set data [format "M %.3f %.3f A" $x1 $y1]
    append data [format " %.3f %.3f %.3f %1d %1d %.3f %.3f"  \
    ...
    switch -- $optA(-style) {
        ...
        pieslice {
            append data [format " L %.3f %.3f Z" $cx $cy]
        }
    }
    return $data
}
...
proc can2svg::MakeArcPathNonA {coo opts} {
    ...
    if {[info exists optA(-style)]} {
        ...
        switch -- $optA(-style) {
            ...
            pieslice {
                append data [format " %.3f %.3f Z" $cx $cy]
            }
        }
    } else {
...
# JAT: Not a font name, so make it one
	set font [eval {font create SimileCustomFont} $fontDesc]
        
    }
        # This is a font name now
